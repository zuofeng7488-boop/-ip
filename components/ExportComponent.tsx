...
export const ExportComponent = forwardRef<HTMLDivElement, ExportProps>(({ characters }, ref) => {
  return (
    <div ref={ref} style={{ width: '1240px', backgroundColor: '#0f172a', color: '#e2e8f0', padding: '40px', fontFamily: "'Inter', 'Noto Sans SC', sans-serif" }}>
      <header className="pdf-block" style={{ textAlign: 'center', marginBottom: '40px', borderBottom: '2px solid #334155', paddingBottom: '20px' }}>
      ...
      {characters.map((char, charIndex) => (
        <section key={char.id} style={{ breakBefore: charIndex > 0 ? 'page' : 'auto', marginBottom: '60px', breakInside: 'avoid-page' }}>
          <header className="pdf-block" style={{ display: 'flex', alignItems: 'baseline', gap: '16px', borderBottom: '1px solid #475569', paddingBottom: '16px', marginBottom: '24px' }}>
          ...
          <div className="pdf-block" style={{ backgroundColor: '#1e293b', padding: '20px', borderRadius: '8px', marginBottom: '24px' }}>
          ...
          {char.scenes.map((scene, sceneIndex) => (
            <article key={scene.id} className="pdf-block" style={{ breakInside: 'avoid-page', marginTop: sceneIndex > 0 ? '40px' : '0' }}>
